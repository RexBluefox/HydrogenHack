...
import org.hydrogenhack.util.Utils;
...
public class Mesh {
    ...
    public Mesh color(QuadColor c) {
        long p = verticesPointer;
        int r = c.getAllColors()[0];
        int g = c.getAllColors()[1];
        int b = c.getAllColors()[2];
        int a = c.getAllColors()[3];
        memPutByte(p, (byte) r);
        memPutByte(p + 1, (byte) g);
        memPutByte(p + 2, (byte) b);
        memPutByte(p + 3, (byte) (a * (float) alpha));
