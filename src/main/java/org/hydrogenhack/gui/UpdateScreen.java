...
import org.hydrogenhack.gui.window.widget.*;
...
public class UpdateScreen extends WindowScreen {
	...
	public void init() {
		...
		addWindow(new Window(width / 2 - wd,
				height / 16,
				width / 2 + wd,
				height - height / 16, String.format("BleachHack Update [%s -> %s]", HydrogenHack.VERSION, updateJson.get("name").getAsString()), new ItemStack(Items.MAGENTA_GLAZED_TERRACOTTA)));
