...
import org.hydrogenhack.event.events.EventRenderInGameHud;
import org.hydrogenhack.event.events.EventTick;
import org.hydrogenhack.eventbus.BleachSubscribe;
import org.hydrogenhack.gui.clickgui.UIClickGuiScreen;
import org.hydrogenhack.gui.clickgui.window.UIContainer;
import org.hydrogenhack.gui.clickgui.window.UIWindow;
import org.hydrogenhack.gui.clickgui.window.UIWindow.Position;
...
public class UI extends Module {
	...
	private Text infAuraText = Text.empty();
	private Text testingText = Text.empty();
	...
	public UI() throws IOException {
		...
		// Testing
		container.windows.put("testing",
				new UIWindow(new Position(0.2, 0.8, "server", 0), container,
						() -> getSetting(12).asToggle().getState(),
						() -> new int[] { mc.textRenderer.getWidth(testingText) + 2, 10 },
						(ms, x, y) -> mc.textRenderer.drawWithShadow(ms, testingText, x + 1, y + 1, 0xa0a0a0))
		);
	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asToggle().getChild(3).asToggle().getState()) {
			...
			if (watermarkMode == 0) {
				moduleListText.add(0, HydrogenHack.watermark.getText().append(Text.literal(" " + HydrogenHack.VERSION).styled(s -> s.withColor(TextColor.fromRgb(0xf0f0f0)))));
			} else {
				moduleListText.add(0, Text.literal("\u00a7a> HydrogenHack " + HydrogenHack.VERSION));
			}
		}
