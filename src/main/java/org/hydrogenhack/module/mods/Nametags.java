...
import org.hydrogenhack.command.commands.CmdEntityStats;
...
import org.hydrogenhack.util.render.WorldRenderer;
...
public class Nametags extends Module {
	...
	public List<Text> getPlayerLines(PlayerEntity player) {
		...
		if (getSetting(1).asToggle().getChild(2).asToggle().getState()) { // Name
			mainText.add(((MutableText) player.getName()).formatted(HydrogenHack.friendMang.has(player) ? Formatting.AQUA : Formatting.RED));
		}
